...
func (core *Core) start(ctx context.Context, id string, cf *config.RWCoreFlags) {
	...
	addGRPCBossOpenoltService(ctx, grpcServer, device.GetNewBossOpenoltManager(deviceMgr))
...
func addGRPCBossOpenoltService(ctx context.Context, server *grpcserver.GrpcServer, handler bossopenolt.BossOpenoltServer) {
	logger.Info(ctx, "bossopenolt-grpc-server-created")
