...
func (loader *Loader) Load(ctx context.Context) {
	...
	for _, flow := range flows {
		...
		loader.dba[flow.Id] = &mycustom{flowId: flow.Id}
	}
}
...
func (loader *Loader) LockOrCreate(ctx context.Context, flow *ofp.OfpFlowStats) (*Handle, bool, error) {
	...
	if !have {
		...
		dbasel := &mycustom{flowId: flow.Id}
		loader.flows[flow.Id] = entry
		logger.Debugw(ctx, "Save my custom struct value Before", log.Fields{"mycustom": loader.dba[flow.Id]})
		loader.dba[flow.Id] = dbasel;
		logger.Debugw(ctx, "Save my custom struct value After", log.Fields{"mycustom": loader.dba[flow.Id]})
//		dbasel.lock.Lock()
		entry.lock.Lock()
		loader.lock.Unlock()
...
func (loader *Loader) ListIDs() map[uint64]struct{} {
	...
	test:= make(map[uint64]struct{}, len(loader.dba))
	for id := range loader.flows {
		ret[id] = struct{}{}
		test[id] = struct{}{}
		logger.Debugw(ctx,"Read my custom struct value ", log.Fields{"myStruct value": test[id]})
	}
