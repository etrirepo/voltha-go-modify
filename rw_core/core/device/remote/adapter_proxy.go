...
import (
	...
	"github.com/opencord/voltha-protos/v4/go/bossopenolt"
)
...
func (ap *AdapterProxy)GetCustomVlan(ctx context.Context, adapterType string, request *bossopenolt.BossRequest) (chan *kafka.RpcResponse, error) {
	logger.Debugw(ctx, "BossRequestVlan", log.Fields{"device-id": request.DeviceId})
	rpc := "boss_get_vlan"
	toTopic, err := ap.getAdapterTopic(ctx, request.DeviceId, adapterType)
	if err != nil {
		return nil, err
	}
	...
	replyToTopic := ap.getCoreTopic()
	return ap.sendRPC(ctx, rpc, toTopic, &replyToTopic, true, request.DeviceId, args...)
}
