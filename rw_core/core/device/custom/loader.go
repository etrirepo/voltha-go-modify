...
func (loader *Loader) Load(ctx context.Context) {
	...
	if err := loader.dbProxy.List(ctx, &flows); err != nil {
		logger.Errorw(ctx, "Custom.....!!!failed-to-list-flows-from-cluster-data-proxy", log.Fields{"error": err})
		return
	}
...
func (loader *Loader) LockOrCreate(ctx context.Context, flow *ofp.OfpFlowStats) (*Handle, bool, error) {
	...
	loader.lock.Lock()
	flowCustomId := flow.Id+1
	entry, have := loader.flows[flowCustomId]
	//dbasel, have := loader.dba[flow.Id]
	if !have {
		entry := &chunk{flow: flow}
		loader.flows[flowCustomId] = entry
		logger.Debugw(ctx, "Save my custom Loader Value", log.Fields{"mycustom": loader.flows[flowCustomId]})
//		dbasel.lock.Lock()
		entry.lock.Lock()
		loader.lock.Unlock()

		if err := loader.dbProxy.Set(ctx, fmt.Sprint(flowCustomId), flow); err != nil {
			// revert the map
			loader.lock.Lock()
			delete(loader.flows, flowCustomId)
			loader.lock.Unlock()
...
//		dbsel.lock.Unlock()
		entry.lock.Unlock()
		return loader.Lock(flowCustomId)
	}
...
func (loader *Loader) ListIDs() map[uint64]struct{} {
	...
	for id := range loader.flows {
		ret[id] = struct{}{}
		logger.Debugw(ctx,"Read my custom Loader Value ", log.Fields{"myLoader value": ret[id]})
	}
		//logger.Debugw(ctx,"mycustom loader uint64 value ", log.Fields{"mycustom": loader.mycustom})
	return ret
}
