...
import (
	...
	"github.com/opencord/voltha-go/rw_core/core/device/custom"
...
func newAgent(ap *remote.AdapterProxy, device *voltha.Device, deviceMgr *Manager, dbPath *model.Path, deviceProxy *model.Proxy, timeout time.Duration) *Agent {
	...
	return &Agent{
		...
		customLoader:	      custom.NewLoader(dbPath.SubPath("custom").Proxy(deviceID)),
...
func (agent *Agent) getCustomVlan(ctx context.Context, reqMessage *bossopenolt.BossRequest) (*bossopenolt.GetVlanResponse, error) {
	...
	//send request to adapter
	ch, err := agent.adapterProxy.GetCustomVlan(ctx, cloned.Adapter, reqMessage)
	agent.requestQueue.RequestComplete()
	if err != nil {
		return nil, err
	}
	...
	rpcResponse, ok := <-ch
	if !ok {
		return nil, status.Errorf(codes.Aborted, "custom getvlanerror", agent.deviceID)
	}
